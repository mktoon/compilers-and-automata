       ...
       switch (p->type) {
        case A_VARDEC : //this is a case for variable declarations
                      ...
                      if(p->value >0){//p.value >0 means its an array
                        printf("Variable %s %s [%d]\n", ASTtypeToString(p->my_data_type), p->name, p->value);//prints out data type, name and val inside the sqre brackets
                      }else{//regular variable
                      printf("Variable %s %s", ASTtypeToString(p->my_data_type), p->name);//prints out var type, and name
                      printf("\n");
                      }//end if else
        ...
        case A_PARAM://case for parameter definition inside function declarations
                    ...
                    if(p->value == 0){//for regular parameter it int x, void y
                        printf("Parameter %s %s\n", ASTtypeToString(p->my_data_type), p->name);//prints out parameter type, and name
                    }
        ...
        case A_FUNCTIONDEC : //case for function declaration 
                    PT(level);
                    printf("Function  %s %s ",ASTtypeToString(p->my_data_type), p->name);//prints out function type and name
                    printf("\n");
        ...
        case A_VAR://case for variables
                  ...
                  if(p->s1 != NULL){//s1 is null because it has no expression
                        printf("VARIABLE %s \n",p->name);//prints the variable name in this case an array var
                        PT(level+1);
                        printf("[\n");
